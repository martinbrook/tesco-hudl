...
static void snd_pcm_show_time(ktime_t starttime, u64 msecs64, char *info)
{
	ktime_t calltime;
	u64 usecs64;
	int usecs;
	...
	if (usecs == 0)
		usecs = 1;
	if ((usecs / USEC_PER_MSEC) > msecs64)
		pr_info("PCM: %s: %ld.%03ld msecs\n", info ? info : " ",
				   ...
				   unsigned int cmd, void __user *arg)
{
	...
	case SNDRV_PCM_IOCTL_WRITEI_FRAMES:
	{
		...
		snd_pcm_show_time(starttime, 
			         (runtime->periods -1)*(runtime->period_size*1000)/44100,
			         "period frames wait too long time");
