...
static char const *const inv_class_name = "gyroscope";
static char const *const inv_device_name = "gyroscope";
static int create_sysfs_interfaces(struct device *dev)
{
	...
	for (i = 0; i < ARRAY_SIZE(attributes); i++)
		if (device_create_file(inv_dev, attributes + i))
			goto error;
	return 0;

error:
	for (; i >= 0; i--)
		device_remove_file(inv_dev, attributes + i);
