/* 
 * drivers/input/touchscreen/ft5606_0x72_ts.c
 *
 * FocalTech ft5606_0x72_ TouchScreen driver. 
 *
...
#define CONFIG_FT5X06_0X72_MULTITOUCH  1
#define MAX_POINT                 10
#define ft5606_0x72_IIC_SPEED          100*1000    //300*1000
//#define TOUCH_RESET_PIN           RK30_PIN1_PC0
#define FT5X06_0X72_REG_THRES          0x80         /* Thresshold, the threshold be low, the sensitivy will be high */
#define FT5X06_0X72_REG_REPORT_RATE    0x88         /* **************report rate, in unit of 10Hz **************/
#define FT5X06_0X72_REG_PMODE          0xA5         /* Power Consume Mode 0 -- active, 1 -- monitor, 3 -- sleep */    
#define FT5X06_0X72_REG_FIRMID         0xA6         /* ***************firmware version **********************/
#define FT5X06_0X72_REG_NOISE_MODE     0xb2         /* to enable or disable power noise, 1 -- enable, 0 -- disable */
#define SCREEN_MAX_X              900//
#define SCREEN_MAX_Y              1440//
#define PRESS_MAX                 255
#define FT5X06_0X72_NAME	              "ft5606_0x72_ts"
#define TOUCH_MAJOR_MAX           200
#define WIDTH_MAJOR_MAX           200
//FT5X06_0X72_REG_PMODE
...
#define FT5X06_0X72_FW_UPGRADE_ASYNC         (1)
#define FT5X06_0X72_FW_UPGRADE_DIRECTLY      (2)

#ifdef CONFIG_MACH_RK3066B_M701
#define FT5X06_0X72_FW_UPGRADE_MODE          FT5X06_0X72_FW_UPGRADE_ASYNC
#else
#define FT5X06_0X72_FW_UPGRADE_MODE          FT5X06_0X72_FW_UPGRADE_DIRECTLY
#endif
...
#ifdef CONFIG_HAS_EARLYSUSPEND
	struct early_suspend ft5606_0x72_early_suspend;
#endif
#if (FT5X06_0X72_FW_UPGRADE_MODE == FT5X06_0X72_FW_UPGRADE_ASYNC)
...
static u8 CTPM_FW[]=
{
#include "ft5606_0x72_goldenfinger.cfg"
};
...
static int fts_i2c_txdata(u8 *txdata, int length)
{
	...
	struct i2c_msg msg;
      ...
      msg.len = length;
      msg.buf = txdata;
      msg.scl_rate = ft5606_0x72_IIC_SPEED;
...
[return]:
			    ...
			    ERR_ECC:        ecc error.
************************************************************************/
E_UPGRADE_ERR_TYPE  fts_0x72_ctpm_fw_upgrade(u8* pbt_buf, int dw_lenth)
{
    u8  cmd,reg_val[2] = {0};
	u8  buffer[2] = {0};
...
int fts_0x72_ctpm_auto_clb(void)
{
...
int fts_0x72_ctpm_fw_upgrade_with_i_file(void)
{
   u8*     pbt_buf = 0;
   int i_ret;
    
   pbt_buf = CTPM_FW;
   i_ret =  fts_0x72_ctpm_fw_upgrade(pbt_buf,sizeof(CTPM_FW));
   if (i_ret != 0)
   {
   ...
   else
   {
       ...
       fts_0x72_ctpm_auto_clb();  //start auto CLB
   }
   return i_ret;
}
...
unsigned char fts_0x72_ctpm_get_upg_ver(void)
{
    unsigned int ui_sz;
...
static int ft5606_0x72_read_regs(struct i2c_client *client, u8 reg, u8 *buf, unsigned len)
{
	int ret; 
	ret = i2c_master_reg8_recv(client, reg, buf, len, ft5606_0x72_IIC_SPEED);
	return ret; 
}

/* set the it7260 registe,used i2c bus*/
static int ft5606_0x72_set_regs(struct i2c_client *client, u8 reg, u8 *buf, unsigned short len)
{
	int ret; 
	ret = i2c_master_reg8_send(client, reg, buf, (int)len, ft5606_0x72_IIC_SPEED);
	return ret;
}

#if (FT5X06_0X72_FW_UPGRADE_MODE == FT5X06_0X72_FW_UPGRADE_ASYNC)
static void ft5606_0x72_firmware_upgrade_work(struct work_struct *work)
{
    struct ft5606_0x72_ts_data *data = container_of(work, struct ft5606_0x72_ts_data, fw_upgrade_work);
    int err = 0;
    ...
    fts_register_read(FT5X06_0X72_REG_FIRMID, &reg_version,1);
    printk("cdy == [TSP] firmware version = 0x%2x\n", reg_version);

    fts_register_read(FT5X06_0X72_REG_FIRMID, &reg_version,1);
    FTprintk("[TSP] firmware version = 0x%2x\n", reg_version);
    if (fts_0x72_ctpm_get_upg_ver() != reg_version)  
    {
      FTprintk("[TSP] start upgrade new verison 0x%2x\n", fts_0x72_ctpm_get_upg_ver());
      msleep(200);
      err =  fts_0x72_ctpm_fw_upgrade_with_i_file();
      if (err == 0)
      {
          FTprintk("[TSP] ugrade successfuly.\n");
          msleep(300);
          fts_register_read(FT5X06_0X72_REG_FIRMID, &reg_value,1);
...
static void ft5606_0x72_queue_work(struct work_struct *work)
{
	struct ft5606_0x72_ts_data *data = container_of(work, struct ft5606_0x72_ts_data, pen_event_work);
...
#if CONFIG_FT5X06_0X72_MULTITOUCH
	ret = ft5606_0x72_read_regs(data->client,start_reg, buf, 6*MAX_POINT+1);
#else
	ret = ft5606_0x72_read_regs(data->client,start_reg, buf, 7);
#endif
	if (ret < 0) {
		dev_err(&data->client->dev, "ft5606_0x72_read_regs fail:%d!\n",ret);
		enable_irq(data->irq);
		return;
	}
#if 0
	...
	//dev_info(&data->client->dev, "ft5606_0x72_read_and_report_data points = %d\n",points);
	if (points == 0) {
#if   CONFIG_FT5X06_0X72_MULTITOUCH
...
static irqreturn_t ft5606_0x72_interrupt(int irq, void *dev_id)
{
	struct ft5606_0x72_ts_data *ft5606_0x72_ts = dev_id;
	FTprintk("[TSP]  ft5606_0x72_interrupt\n");
	disable_irq_nosync(ft5606_0x72_ts->irq);
	if (!work_pending(&ft5606_0x72_ts->pen_event_work)) 
		queue_work(ft5606_0x72_ts->ts_workqueue, &ft5606_0x72_ts->pen_event_work);
	return IRQ_HANDLED;
}
#ifdef CONFIG_HAS_EARLYSUSPEND
#define TOUCH_RESET_PIN  RK30_PIN0_PB6

static void ft5606_0x72_suspend(struct early_suspend *h)
{
	struct ft5606_0x72_ts_data *ft5606_0x72_ts;
	char buf_w[1] = {3};
	int err;
    ft5606_0x72_ts = container_of(h, struct ft5606_0x72_ts_data, ft5606_0x72_early_suspend);
	FTprintk("TSP ft5606_0x72_suspend\n");
    err = ft5606_0x72_set_regs(this_client, 0xA5, buf_w, 1);
    if (err > 0)
        printk("ft5606_0x72_set_regs OK!!\n");
    #if 0
    msleep(20);

	if (ft5606_0x72_ts->platform_sleep){ 
		ft5606_0x72_ts->platform_sleep();
	}
    #endif
	gpio_set_value(TOUCH_RESET_PIN,GPIO_LOW);
	disable_irq(ft5606_0x72_ts->irq);
}

static void ft5606_0x72_resume(struct early_suspend *h)
{
	struct ft5606_0x72_ts_data *ft5606_0x72_ts;	
	int trytimes;
	u8 val = 0;
	ft5606_0x72_ts = container_of(h, struct ft5606_0x72_ts_data, ft5606_0x72_early_suspend);
	FTprintk("TSP ft5606_0x72_resume\n");
	gpio_set_value(TOUCH_RESET_PIN,GPIO_LOW);
	msleep(10);
	gpio_set_value(TOUCH_RESET_PIN,GPIO_HIGH);
	mdelay(200);
	enable_irq(ft5606_0x72_ts->irq);
	#if 0
	if (ft5606_0x72_ts->platform_wakeup)                              
		ft5606_0x72_ts->platform_wakeup();
		
     
	for(trytimes = 0 ;trytimes < 5; trytimes++){
		if(ft5606_0x72_read_regs(this_client, 0x00,  &val,1)<0){
			if (ft5606_0x72_ts->platform_sleep) 
				ft5606_0x72_ts->platform_sleep();
			if (ft5606_0x72_ts->platform_wakeup)                              
				ft5606_0x72_ts->platform_wakeup();
...
static int __devexit ft5606_0x72_remove(struct i2c_client *client)
{
	struct ft5606_0x72_ts_data *ft5606_0x72_ts = i2c_get_clientdata(client);

	free_irq(ft5606_0x72_ts->irq, ft5606_0x72_ts);
	input_unregister_device(ft5606_0x72_ts->input_dev);
	kfree(ft5606_0x72_ts);

	#ifdef FTS_CTL_IIC
	ft_rw_iic_drv_exit();
	#endif

	cancel_work_sync(&ft5606_0x72_ts->pen_event_work);
	destroy_workqueue(ft5606_0x72_ts->ts_workqueue);
	i2c_set_clientdata(client, NULL);
#ifdef CONFIG_HAS_EARLYSUSPEND
    unregister_early_suspend(&ft5606_0x72_ts->ft5606_0x72_early_suspend);
#endif 
    this_client = NULL;
	return 0;
}

static int  ft5606_0x72_probe(struct i2c_client *client ,const struct i2c_device_id *id)
{
	struct ft5606_0x72_ts_data *ft5606_0x72_ts;
	struct input_dev *input_dev;
	...
	dev_info(&client->dev, "ft5606_0x72_ts_probe!\n");
	if (TouchPanelFound == 1){
		printk("Already found Touch Panel, no need to check I2C 0x72");
		return -EINVAL;
	}
	if (!pdata) {
	...
	ft5606_0x72_ts = kzalloc(sizeof(*ft5606_0x72_ts), GFP_KERNEL);
	ft5606_0x72_ts->platform_wakeup = pdata->platform_wakeup;
	ft5606_0x72_ts->platform_sleep = pdata->platform_sleep;	
	if (!ft5606_0x72_ts)	{
		return -ENOMEM;
	}
    ...
    while(retry < 2) 
    {    
        ret = ft5606_0x72_set_regs(client,FT5X06_0X72_REG_PMODE, buf_test, 1);
        if(ret > 0) 
            break;
        retry++;

        printk("FT5606_0X72 I2C TEST FAILED, retry = %d, ret = %d, will again...\n", retry, ret);
    ...
    printk("FT5606_0X72 I2C TEST OK, retry = %d, ret = %d\n", retry, ret);

    if(ret <= 0)
    {    
        printk("FT5606_0X72 I2C TEST ERROR! retry = %d, ret = %d\n", retry, ret);
        err = -ENODEV;
	...
	ft5606_0x72_ts->client = this_client = client;
	ft5606_0x72_ts->irq = client->irq;
	ft5606_0x72_ts->input_dev = input_dev;
	...
	input_dev->name		= "ft5606_0x72_ts-touchscreen";		//dev_name(&client->dev)
	err = input_register_device(input_dev);
	if (err) {
		printk("ft5606_0x72_ts_probe: failed to register input device: \n");
		goto exit_input_register_device_failed;
	}

	if (!ft5606_0x72_ts->irq) {
		err = -ENODEV;
		dev_err(&ft5606_0x72_ts->client->dev, "no IRQ?\n");
		goto exit_no_irq_fail;
	}else{
		ft5606_0x72_ts->irq = gpio_to_irq(ft5606_0x72_ts->irq);
	}
#ifdef FTS_CTL_IIC
	...
	INIT_WORK(&ft5606_0x72_ts->pen_event_work, ft5606_0x72_queue_work);
	ft5606_0x72_ts->ts_workqueue = create_singlethread_workqueue("ft5606_0x72_ts");
	if (!ft5606_0x72_ts->ts_workqueue) {
		err = -ESRCH;
...
/* zhengxing: will upgrade firmware async */
    ...
    ft5606_0x72_ts->fw_workqueue = create_singlethread_workqueue("ft5606_0x72_fw");
    if (!ft5606_0x72_ts->fw_workqueue) {
        err = -ESRCH;
        goto exit_create_singlethread;
    }
    INIT_WORK(&ft5606_0x72_ts->fw_upgrade_work, ft5606_0x72_firmware_upgrade_work);    
#else
...
#endif
#endif
	ret = request_irq(ft5606_0x72_ts->irq, ft5606_0x72_interrupt, IRQF_TRIGGER_FALLING, client->dev.driver->name, ft5606_0x72_ts);
	if (ret < 0) {
		dev_err(&client->dev, "irq %d busy?\n", ft5606_0x72_ts->irq);
		goto exit_irq_request_fail;
	}
	i2c_set_clientdata(client, ft5606_0x72_ts);
#ifdef CONFIG_HAS_EARLYSUSPEND
	ft5606_0x72_ts->ft5606_0x72_early_suspend.suspend =ft5606_0x72_suspend;
	ft5606_0x72_ts->ft5606_0x72_early_suspend.resume =ft5606_0x72_resume;
	ft5606_0x72_ts->ft5606_0x72_early_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN + 1;;
	register_early_suspend(&ft5606_0x72_ts->ft5606_0x72_early_suspend);
#endif
	buf_w[0] = 6;
	err = ft5606_0x72_set_regs(client,0x88,buf_w,1);
	buf_r[0] = 0;
	err = ft5606_0x72_read_regs(client,0x88,buf_r,1);
	FTprintk("read buf[0x88] = %d\n", buf_r[0]);

#if (FT5X06_0X72_FW_UPGRADE_MODE == FT5X06_0X72_FW_UPGRADE_ASYNC) 
    queue_work(ft5606_0x72_ts->fw_workqueue, &ft5606_0x72_ts->fw_upgrade_work);
#endif
...
static struct i2c_device_id ft5606_0x72_idtable[] = {
	{ FT5X06_0X72_NAME, 0 },
	{ }
};

MODULE_DEVICE_TABLE(i2c, ft5606_0x72_idtable);

static struct i2c_driver ft5606_0x72_driver  = {
	.driver = {
		.owner	= THIS_MODULE,
		.name	= FT5X06_0X72_NAME
	},
	.id_table	= ft5606_0x72_idtable,
	.probe      = ft5606_0x72_probe,
    //.suspend	= ft5606_0x72_suspend,
	//.resume	    = ft5606_0x72_resume,
	.remove 	= __devexit_p(ft5606_0x72_remove),
};

static int __init ft5606_0x72_ts_init(void)
{
	return i2c_add_driver(&ft5606_0x72_driver);
}

static void __exit ft5606_0x72_ts_exit(void)
{
	FTprintk("Touchscreen driver of ft5306 exited.\n");
	i2c_del_driver(&ft5606_0x72_driver);
}


/***********************************************************************/

module_init(ft5606_0x72_ts_init);
module_exit(ft5606_0x72_ts_exit);

MODULE_AUTHOR("<wenfs@Focaltech-systems.com>");
MODULE_DESCRIPTION("FocalTech ft5606_0x72_ TouchScreen driver");
