...
static void gtp_touch_down(struct novatek_ts_data* ts,s32 id,s32 x,s32 y,s32 w)
{
...
Output:
	None.
*******************************************************/
static void gtp_touch_up(struct novatek_ts_data* ts, s32 id)
{
    input_mt_slot(ts->input_dev, id);
    input_report_abs(ts->input_dev, ABS_MT_TRACKING_ID, -1);
    KFprintk("Touch id[%2d] release!\n", id);
}
...
{	
	...
	if(touch_num)
	{
		for(index=0; index<MAX_FINGER_NUM; index++)
		{
            ...
            input_tmp = input_x;
            input_x = ts->abs_x_max - input_y;
            input_y = input_tmp;

			if((input_x > ts->abs_x_max) || (input_x < 0) ||(input_y > ts->abs_y_max) || (input_y < 0))
            {
                gtp_touch_up(ts, index);
                continue;
			}
			//if((input_x > SCREEN_MAX_WIDTH)||(input_y > SCREEN_MAX_HEIGHT))continue;
			//printk("input_x = %d,input_y = %d, input_w = %d\n track_id[%d]=%d \n", input_x, input_y, input_w , index,track_id[index]);
			KFprintk("id = [%x]\n", point_data[position]);
			if((point_data[position]&0x3)==1 || (point_data[position]&0x3)==2)gtp_touch_down(ts, track_id[index],input_x,input_y,input_w);
            else gtp_touch_up(ts, track_id[index]);
		}
	}

	#ifdef HAVE_TOUCH_KEY
	KFprintk("HAVE KEY DOWN!0x%x\n",point_data[1]);
...
static int novatek_suspend(struct i2c_client *client, pm_message_t mesg)
{
    ...
    printk(KERN_ERR " novatek_ts_suspend   ts->use_irq = %d\n",ts->use_irq);
    if (ts->use_irq)
...
static int novatek_resume(struct i2c_client *client)
{
	...
	if(pdata->do_reset)
		pdata->do_reset();
	printk(KERN_INFO "novatek_ts_resume   ts->use_irq = %d\n",ts->use_irq);
	if (ts->use_irq)
		enable_irq(client->irq);
	else
...
static int	nvctp_CheckIsBootloader(struct i2c_client *client)
{
	...
	printk("nvctp_CheckIsBootloader version = %x,%x,%x,%x\n",iic_data_buffer[1],iic_data_buffer[0],iic_data_buffer[2],write_cmd1[1]);

	iic_data_buffer[0]=0x00;		
	iic_data_buffer[1] = 0x99;
	iic_data_buffer[2] = (unsigned char)(FW_CHECKSUM_ADDR >> 8);
	iic_data_buffer[3] = FW_CHECKSUM_ADDR&0xFF;
	iic_data_buffer[4] = 8;
	BootLoader_write_bytes(ts->client,iic_data_buffer,5);
	nvctp_DelayMs(2);
	BootLoader_read_bytes(ts->client,iic_data_buffer,14);
...
{
	#include "novatek_v325_v03_20121112_CSM_7C03.fw"
...
static int	nvctp_CheckIsBootloader(struct i2c_client *client)
{
	...
	uint16_t FW_CHECKSUM ;
	uint8_t iic_buffer[3];
	KFprintk("nvctp_CheckIsBootloader\n");
		   
	FW_CHECKSUM = (unsigned int)(nvctp_BinaryFile[FW_DATASIZE-2]<< 8 |nvctp_BinaryFile[FW_DATASIZE-1]);
	KFprintk("%s,  new version checksum = %x \n",__func__,FW_CHECKSUM );
	if(nvctp_ReadFinalChecksum(FW_CHECKSUM_ADDR,FW_CHECKSUM,ts->client))
	{
		printk("nvctp_CheckIsBootloader1\n");
		novatek_Bootloader(nvctp_BinaryFile,sizeof(nvctp_BinaryFile),ts->client);
	...
	{
		KFprintk("--%s--, tp version is no change\n",__func__);
		iic_buffer[0] = 0x00;
		iic_buffer[1] = 0xA5;
		BootLoader_write_bytes(ts->client, iic_buffer, 2);										
	}
