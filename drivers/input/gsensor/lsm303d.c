				     	...
				     	char *buf)
{
	...
	val = stat->pdata_acc->poll_interval / 1000;
				     	...
				     	char *buf)
{
	...
	struct lsm303d_status *stat = dev_get_drvdata(dev);
	mutex_lock(&stat->lock);
	val = stat->pdata_mag->poll_interval / 1000;
	mutex_unlock(&stat->lock);
	return sprintf(buf, "%u\n", val);
}
...
static struct kobj_attribute poll_attr_acc =
	__ATTR(pollrate_ms, 0777, attr_get_polling_rate_acc, 
						attr_set_polling_rate_acc);
static struct kobj_attribute enable_attr_acc =
	__ATTR(enable_device, 0777, attr_get_enable_acc, 
						attr_set_enable_acc);
static struct kobj_attribute fs_attr_acc =
	__ATTR(full_scale, 0777, attr_get_range_acc, attr_set_range_acc);

static struct kobj_attribute aa_filter_attr  =
	__ATTR(anti_aliasing_frequency, 0777, attr_get_aa_filter, 
							attr_set_aa_filter);

static struct kobj_attribute poll_attr_mag =
	__ATTR(pollrate_ms, 0777, attr_get_polling_rate_mag, 
						attr_set_polling_rate_mag);
static struct kobj_attribute enable_attr_mag =
	__ATTR(enable_device, 0777, attr_get_enable_mag, 
						attr_set_enable_mag);
static struct kobj_attribute fs_attr_mag =
	__ATTR(full_scale, 0777, attr_get_range_mag, attr_set_range_mag);
...
static char const *const inv_class_name = "gsensor_compass";
static char const *const inv_device_name = "gsensor_compass";
static int create_sysfs_interfaces(struct device *dev)
{
	int err;
    struct class *inv_class;
    struct device   *inv_dev;
    void *p_driver_data = dev_get_drvdata(dev);

    inv_class = class_create(THIS_MODULE, inv_class_name);
	if (IS_ERR(inv_class)) {
		printk("%s %d err\n",__func__,__LINE__);
		return 0;
	}
	inv_dev = device_create(inv_class, dev,
			NULL, p_driver_data, inv_device_name);
	if (IS_ERR(inv_dev)) {
		printk("%s %d err\n",__func__,__LINE__);
		return 0;
	}

	acc_kobj = kobject_create_and_add("accelerometer", &inv_dev->kobj);
	if(!acc_kobj)
		return -ENOMEM;

	mag_kobj = kobject_create_and_add("magnetometer", &inv_dev->kobj);
	if(!mag_kobj)
		return -ENOMEM;
...
static int lsm303d_acc_get_data(struct lsm303d_status *stat, int *xyz)
{
	...
	hw_d[0] = hw_d[0] * stat->sensitivity_acc/1000;
	hw_d[1] = hw_d[1] * stat->sensitivity_acc/1000;
	hw_d[2] = hw_d[2] * stat->sensitivity_acc/1000;
...
static int lsm303d_mag_get_data(struct lsm303d_status *stat, int *xyz)
{
	...
	xyz[2] = ((stat->pdata_mag->negate_z) ? 
				(-hw_d[stat->pdata_mag->axis_map_z])
		   			: (hw_d[stat->pdata_mag->axis_map_z]));
//    printk("x=%d, y=%d, z=%d, sensitivity_mag=%d\n",xyz[0]/1000,xyz[1]/1000,xyz[2]/1000,stat->sensitivity_mag);
	return err;
}
