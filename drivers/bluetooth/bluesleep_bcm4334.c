...
//                   tegra_uart_request_clock_off(bsi->uport);
    ...
    int bluesleep_can_sleep(void)
    {
			...
			BT_DBG("host_wake != bsi->irq_polarity ? : %d", (gpio_get_value(bsi->host_wake) != bsi->irq_polarity));		// added by brcm yjhuang 2013-04-24
			BT_DBG("!test_bit(BT_EXT_WAKE, &flags)? : %d", !test_bit(BT_EXT_WAKE, &flags));		// added by brcm yjhuang 2013-04-24
			BT_DBG("bsi->uport != NULL ? : %d", bsi->uport != NULL);		// added by brcm yjhuang 2013-04-24
    ...
    static void bluesleep_sleep_work(struct work_struct *work)
    {
            ...
            if (bluesleep_can_sleep()) {
					BT_DBG("bluesleep can sleep");									// added by brcm yjhuang 2013-04-24
					...
					txEmptyFlag = bsi->uport->ops->tx_empty(bsi->uport);		// added by brcm yjhuang 2013-04-24
					BT_DBG("txEmptyFlag = %d", txEmptyFlag);	
                    ...
                    if (txEmptyFlag) {
							...
							BT_DBG("tx is not empty, continue to delay timer");									// added by brcm yjhuang 2013-04-24
                            mod_timer(&tx_timer, jiffies + TX_TIMER_INTERVAL * HZ);
                            return;
                    }
            } else if (!test_bit(BT_EXT_WAKE, &flags)
                            && !test_bit(BT_ASLEEP, &flags)) {
					BT_DBG("== set bt exit wake ==");								// added by brcm yjhuang 2013-04-24
                    mod_timer(&tx_timer, jiffies + (TX_TIMER_INTERVAL * HZ));
                    if (bsi->has_ext_wake == 1)
                            gpio_set_value(bsi->ext_wake, 1);
                    set_bit(BT_EXT_WAKE, &flags);
            } else {
					BT_DBG("== call bluesleep wakeup ==");							// added by brcm yjhuang 2013-04-24
    ...
    static int bluesleep_start(void)
    {
			...
			enable_irq(bsi->host_wake_irq);//add
			...
			BT_DBG("== bluesleep start ==");				// added by yjhuang 2013-04-24
            return 0;
    fail:
            del_timer(&tx_timer);
            atomic_inc(&open_count);
    ...
    static void bluesleep_stop(void)
    {
			...
			BT_DBG("== bluesleep stop ==");				// added by yjhuang 2013-04-24
    }
    /**
    ...
    static int bluesleep_probe(struct platform_device *pdev)
    {
            ...
            bsi->host_wake_irq = gpio_to_irq(res->start);
            ...
            if (bsi->irq_polarity == POLARITY_LOW) {
					BT_DBG("irq_polarity = POLARITY_LOW");						// added by brcm yjhuang 2013-0425
                    ret = request_irq(bsi->host_wake_irq, bluesleep_hostwake_isr,
                                    IRQF_DISABLED | IRQF_TRIGGER_FALLING,
                                    "bluetooth hostwake", NULL);
            } else {
            		BT_DBG("irq_polarity = POLARITY_HIGH");						// added by brcm yjhuang 2013-0425
                    ret = request_irq(bsi->host_wake_irq, bluesleep_hostwake_isr,
                                    IRQF_DISABLED | IRQF_TRIGGER_RISING,
                                    "bluetooth hostwake", NULL);
            }
			disable_irq(bsi->host_wake_irq);//add
			ret = enable_irq_wake(bsi->host_wake_irq);//add
			if (ret) goto free_bsi;
            if (ret  < 0) {
