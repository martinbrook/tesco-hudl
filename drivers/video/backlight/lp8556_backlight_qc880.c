...
#ifdef CONFIG_LCD_AUO_AND_LG_MIPI_QC880
extern int is_lg_ld070wx5_i2c_exist(void);
#endif
...
int lp8556_backlight_hw_init(struct i2c_client *client)
{
...
#ifdef CONFIG_LCD_AUO_AND_LG_MIPI_QC880
    uint8_t Write_data1_lg[] ={0xa1, 0x74}; //hight bit(8~11)(0~0X66e set backlight)
    uint8_t Write_data2_lg[] ={0xa0, 0xF5};  //low bit(0~7)  15.5mA
    uint8_t Write_data3_lg[] ={0x16, 0x0f}; // 4channel LED enable
    uint8_t Write_data4_lg[] ={0xa9, 0x80}; //VBOOST_MAX 21V
#endif
    ...
    {
        DBG("Using LG Backlight Config\n");
        i2c_write_bytes(client,Write_data1_lg,2);
        i2c_write_bytes(client,Write_data2_lg,2);
        i2c_write_bytes(client,Write_data3_lg,2);
        i2c_write_bytes(client,Write_data4_lg,2);
    }
    else
    {
        DBG("Using AUO Backlight Config\n");
