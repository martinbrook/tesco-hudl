...
static struct rk_fb_inf *fb_in;

void rk29_backlight_set(bool on);

static u32 g_y_addr = 0;
static u32 g_uv_addr =0;

unsigned long g_smem_start = 0;
u32 g_smem_len = 0;
char __iomem *g_screen_base = NULL;
			...
			if(hdmi_get_hotplug() == HDMI_HPD_ACTIVED)
			{
				if(inf->num_fb >= 2)
				{
					...
					if(!((par->format == YUV420 || par->format ==YUV422)&&(var->rotate != 0))){
						...
						par2->y_addr = par->smem_start + par->y_offset;
						par2->uv_addr = par->cbr_start + par->c_offset;
					}
...
static int rk_fb_set_par(struct fb_info *info)
{
	...
	if(dev_drv->screen_ctr_info->prop == EXTEND){
		xsize = dev_drv->cur_screen->xsize;
		ysize = dev_drv->cur_screen->ysize;
		xpos = dev_drv->cur_screen->xpos;
		ypos = dev_drv->cur_screen->ypos;
	}
	...
	par->yvir =  var->yres_virtual;
			...
			if(hdmi_get_hotplug() == HDMI_HPD_ACTIVED)
			{
				if(info != info2 && info2!=NULL)
				{
					...
					{
						par2 = dev_drv1->layer_par[layer_id];
						if((par->xact < par->yact)&&!((par->format == YUV420 || par->format ==YUV422)&&(var->rotate !=0) &&(info != info2)))
						{
							par2->xact = par->yact;
							par2->yact = par->xact;
							par2->xvir = par->yact;
						...
						if((var->rotate == 90 || var->rotate == 270)&&(par->format == YUV420 || par->format ==YUV422)){
							par2->xsize = par->ysize*dev_drv1->cur_screen->xsize/screen->y_res;
							par2->ysize = par->xsize*dev_drv1->cur_screen->ysize/screen->x_res;
							par2->xpos = par->ypos*dev_drv1->cur_screen->xsize/screen->y_res + dev_drv1->cur_screen->xpos;
							par2->ypos = par->xpos*dev_drv1->cur_screen->ysize/screen->x_res + dev_drv1->cur_screen->ypos;
						}else{
							par2->xsize = dev_drv1->cur_screen->xsize;
							par2->ysize = dev_drv1->cur_screen->ysize;
							par2->xpos = dev_drv1->cur_screen->xpos;
							par2->ypos = dev_drv1->cur_screen->ypos;
						}
	...
	if((par->format == YUV420 || par->format ==YUV422)&&(var->rotate !=0) &&(info != info2)){
		static int flip=0;
		flip++;
		if(flip == 2)
			flip=0;
		...
		ipp_req.src0.YrgbMst = par->smem_start + par->y_offset;
		ipp_req.src0.CbrMst = par->cbr_start + par->c_offset;
		ipp_req.src0.w = info->var.xres;
		ipp_req.src0.h = info->var.yres;

		if(par->format == YUV422){
		...
		ipp_req.dst0.YrgbMst = g_smem_start + info->var.xres*info->var.yres*2*flip;
		ipp_req.dst0.CbrMst = g_smem_start+ info->var.xres*info->var.yres*(2*flip+1);
		if(var->rotate == 90 || var->rotate == 270){
			ipp_req.dst0.w = info->var.yres;// screen->x_res;
			if(info->var.yres > 1080){
				ipp_req.dst0.h = 1080;
			}else{
				ipp_req.dst0.h = info->var.xres;
			}
			par->xact = ipp_req.dst0.w;//var->yres;              //winx active window height,is a part of vir
			par->yact = ipp_req.dst0.h;//var->xres;
			par->xvir = (ipp_req.dst0.w + 15)&(~15);// var->xres_virtual;		// virtual resolution	 stride --->LCDC_WINx_VIR
			par->yvir = (ipp_req.dst0.h + 15)&(~15);// var->yres_virtual;
		}else{
			ipp_req.dst0.w = info->var.xres;// screen->x_res;
			ipp_req.dst0.h = info->var.yres;//screen->y_res;
		}

		ipp_req.src_vir_w = info->var.xres_virtual;
		...
		ipp_blit_sync(&ipp_req);
		g_y_addr = ipp_req.dst0.YrgbMst;
		g_uv_addr = ipp_req.dst0.CbrMst;
	}
...
int rk_fb_disp_scale(u8 scale_x, u8 scale_y,u8 lcdc_id)
{
	...
	struct rk_lcdc_device_driver * dev_drv_hdmi = NULL;
	struct rk_lcdc_device_driver * dev_drv_lcd = NULL;
	rk_screen *lcd_screen = NULL;
	rk_screen *hdmi_screen = NULL;
...
#if defined(CONFIG_ONE_LCDC_DUAL_OUTPUT_INF)
	dev_drv_lcd=dev_drv_hdmi = inf->lcdc_dev_drv[0];
#else
	for(i = 0; i < inf->num_lcdc; i++)
	{
		if(inf->lcdc_dev_drv[i]->screen_ctr_info->prop == EXTEND)
		{
			dev_drv_hdmi = inf->lcdc_dev_drv[i];
		}
		if(inf->lcdc_dev_drv[i]->screen_ctr_info->prop == PRMRY)
		{
			dev_drv_lcd = inf->lcdc_dev_drv[i];
		}
	}
	if( dev_drv_hdmi==NULL || dev_drv_lcd==NULL)
	{
		printk(KERN_ERR "%s driver not found!",name);
		return -ENODEV;
	...
	{
		info = inf->fb[dev_drv_hdmi->num_layer];
	}
	hdmi_screen = dev_drv_hdmi->cur_screen;
	lcd_screen = dev_drv_lcd->cur_screen;

	if(lcd_screen->x_res > lcd_screen->y_res){
		if(lcd_screen->x_res*hdmi_screen->y_res < lcd_screen->y_res*hdmi_screen->x_res){
			screen_x = hdmi_screen->y_res*lcd_screen->x_res/lcd_screen->y_res;
			screen_y = hdmi_screen->y_res;
		}else{
			screen_x = hdmi_screen->x_res;
			screen_y = hdmi_screen->x_res*lcd_screen->y_res/lcd_screen->x_res;
		}
	}else{
		if(lcd_screen->y_res*hdmi_screen->y_res < lcd_screen->x_res*hdmi_screen->x_res){
			screen_x = hdmi_screen->y_res*lcd_screen->y_res/lcd_screen->x_res;
			screen_y = hdmi_screen->y_res;
		}else{
			screen_x = hdmi_screen->x_res;
			screen_y = hdmi_screen->y_res;
			//screen_y = hdmi_screen->x_res*lcd_screen->x_res/lcd_screen->y_res;
		}
	}
	
	hdmi_screen->xpos = (hdmi_screen->x_res-screen_x*scale_x/100)>>1;
	hdmi_screen->ypos = (hdmi_screen->y_res-screen_y*scale_y/100)>>1;
	hdmi_screen->xsize = screen_x*scale_x/100;
	hdmi_screen->ysize = screen_y*scale_y/100;
...
static int rk_request_fb_buffer(struct fb_info *fbi,int fb_id)
{
	...
	if (!strcmp(fbi->fix.id,"fb0"))
	{
		...
		if (res == NULL)
		{
			dev_err(&g_fb_pdev->dev, "failed to get memory for yuv buf \n");
			ret = -ENOENT;
		}
		g_smem_start = res->start;
		g_smem_len = res->end - res->start + 1;
		mem = request_mem_region(res->start, resource_size(res), g_fb_pdev->name);
		g_screen_base = ioremap(res->start, g_smem_len);
		memset(g_screen_base, 0, g_smem_len);
		printk("fb%d:phy:%lx>>vir:%p>>len:0x%x\n",fb_id,
		g_smem_start,g_screen_base,g_smem_len);
...
#endif
int __sramdata is_find_battery_low_logo = 0;
void clear_fb(struct fb_info *info)
{
	memset(info->screen_base,
		0, info->var.xres*info->var.yres*4);
}
void show_battery_low(void )
{
	is_find_battery_low_logo = 1;
	fb_in->fb[0]->fbops->fb_open(fb_in->fb[0],1);
	fb_in->fb[0]->fbops->fb_set_par(fb_in->fb[0]);

	if(fb_prepare_logo(fb_in->fb[0], FB_ROTATE_UR)) {
		/* Start display and show logo on boot */
		fb_set_cmap(&fb_in->fb[0]->cmap, fb_in->fb[0]);
		clear_fb(fb_in->fb[0]);
		fb_show_logo(fb_in->fb[0], FB_ROTATE_UR);
		fb_in->fb[0]->fbops->fb_pan_display(&(fb_in->fb[0]->var), fb_in->fb[0]);
	}
	fb_in->fb[0]->fbops->fb_ioctl(fb_in->fb[0],RK_FBIOSET_CONFIG_DONE,NULL);
	is_find_battery_low_logo = 0;
}
EXPORT_SYMBOL(show_battery_low);
