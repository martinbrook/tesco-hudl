...
//#define DET_CNT   5
#define PWR_ON_THRESHD 3//99      //power on threshd of capacity
//unsigned int   pre_cnt = 0;   //for long press counter 
//int charge_disp_mode = 0;
static int pwr_on_thrsd = PWR_ON_THRESHD;          //power on capcity threshold

extern __sramdata int PowerKeyPressed;
...
extern void show_battery_low(void);
...
static int  __init start_charge_logo_display(void)
{
	...
	/* michael@kh : original logic */
	/*
	if(val_status.intval == POWER_SUPPLY_STATUS_CHARGING)
	{
		if ((board_boot_mode() != BOOT_MODE_REBOOT) || (val_capacity.intval <= pwr_on_thrsd))  
		//do not enter power on charge mode when soft reset
		if ((board_boot_mode() != BOOT_MODE_REBOOT) || (val_capacity.intval <= pwr_on_thrsd))  
	    {			
	...
	*/
	...
	/* michael@kh : prevent enter Android mode without power key pressed */

	/* michael@kh : do not enter power on charge mode when soft reset */
	

		if( (val_capacity.intval <= pwr_on_thrsd) && (val_status.intval != POWER_SUPPLY_STATUS_CHARGING) )
		{
			printk("low power & no charging!\n");
			show_battery_low();
			msleep(2000);
			kernel_power_off();
			while(1);
...
//not detect power key.
#if 0
		//if ( !PowerKeyPressed ) 
		{
			if (val_status.intval == POWER_SUPPLY_STATUS_CHARGING) {
				printk("Powered by USB so enter charger mode!\n");
				add_bootmode_charger_to_cmdline();
			} else {
				printk("Powered by USB but removed!\n");
				kernel_power_off();
				while(1);
				return 0;
			}
