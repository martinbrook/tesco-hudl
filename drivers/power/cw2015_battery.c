...
static int cw_get_vol(struct cw_battery *cw_bat)
{
        ...
        value16_2 = reg_val[0];
        value16_2 = (value16_2 << 8) + reg_val[1];
...
static int rk_ac_update_online(struct cw_battery *cw_bat)
{
    ...
    xprintk("%s usb_status=[%d],charger_init_mode=[%d],cw_bat->charger_mode=[%d],gadget_status=[%d]\n",__func__,usb_status,cw_bat->charger_init_mode,cw_bat->charger_mode,gadget_status);
        if(gpio_get_value(cw_bat->plat_data->dc_det_pin) == cw_bat->plat_data->dc_det_level) {
            if(((usb_status == 2)||(cw_bat->charger_init_mode==2)) && (cw_bat->charger_mode != AC_CHARGER_MODE))
            {
			    ...
			    return 1;
            //}else if(((usb_status == 1)||(cw_bat->charger_init_mode==1)) && (cw_bat->charger_mode != USB_CHARGER_MODE))
            //Add detection of the so call abnormal AC Adapter
            }else if((usb_status == 1)||(cw_bat->charger_init_mode==1))
            {
            	  if((0 == gadget_status) && (cw_bat->charger_mode != AC_CHARGER_MODE)){
                	cw_update_time_member_ac_online(cw_bat);
                	cw_bat->charger_mode = AC_CHARGER_MODE;
                	gpio_direction_output(cw_bat->plat_data->chg_mode_sel_pin, (cw_bat->plat_data->chg_mode_sel_level==GPIO_HIGH) ? GPIO_HIGH : GPIO_LOW);
				  }else if((1 == gadget_status) && (cw_bat->charger_mode != USB_CHARGER_MODE)){
                	cw_update_time_member_ac_online(cw_bat);
...
static void cw_bat_work(struct work_struct *work)
{
        ...
        xprintk("bat_change = %d, time_to_empty = %d, capacity = %d, voltage = %d, charger_mode = %d\n",\
                        cw_bat->bat_change, cw_bat->time_to_empty, cw_bat->capacity, cw_bat->voltage, cw_bat->charger_mode);
}
...
static int cw_bat_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ret = cw_init(cw_bat);
	while((loop++ <3)&&(ret <0))
        	ret = cw_init(cw_bat);    //当通讯有ACK或者循环次数大于2000时，跳出while
