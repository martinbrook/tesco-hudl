...
static struct rk30_i2c *g_i2c[MAX_NR];
...
static int rk30_i2c_probe(struct platform_device *pdev)
{
	...
	g_i2c[i2c_max_adap++] = i2c;
	BUG_ON(i2c_max_adap > 5);
...
static void rk30_i2c_shutdown(void)
{
	int i;
	if (system_state == SYSTEM_RESTART){
		for(i = 0; i < i2c_max_adap; i++){
			i2c_dbg(g_i2c[i]->dev, "shutdown\n");
			mutex_lock(&g_i2c[i]->m_lock);
		}
	}
}
static struct syscore_ops rk30_i2c_syscore_ops = {
	.shutdown	= rk30_i2c_shutdown,
};
...
{
	register_syscore_ops(&rk30_i2c_syscore_ops);
